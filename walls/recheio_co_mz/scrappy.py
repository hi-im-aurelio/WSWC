...
def scraping(document:BlocExcel, list_item):
    ...
    for ELEMENT in list_item:
        ...
        try:
            product_name = ELEMENT.find_element(By.CLASS_NAME, "ps-product__container").find_element(By.TAG_NAME, "a").get_attribute("title")
        except:
          try:
            product_name = ELEMENT.find_element(By.CLASS_NAME, "ps-product__container").find_element(By.TAG_NAME, "a").text # caso o title falhe
          except:
            product_name = 'Informação não encotrada'
            print('An exception occurred: product_name')

        try:
          product_price = ELEMENT.find_element(By.CLASS_NAME, "ps-product__container").find_element(By.TAG_NAME, "p").text
        except:
          print('An exception occurred: product_price')
        
        try:
          product_page = ELEMENT.find_element(By.CLASS_NAME, "ps-product__thumbnail").find_element(By.TAG_NAME, "a").get_attribute("href")
        except:
